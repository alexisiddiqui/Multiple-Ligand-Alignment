...
A. Plot the global Neff / confidence for each ligand with mean tanimoto similarity between the query and the ligand 
...
B. Then check whether low-Neff atoms correspond to the atoms with highest B-factors

1. Download PDB files for CDK2 + Ligands
2. Extract ligands and compute per-atom Neff using all other PDB ligands for that target as the reference set.
3. Plot Neff against the spearman correlation of the atom-wise Neff values and B-factors across all atoms in the ligand.
